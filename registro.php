<?php
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		if (empty(trim($_POST['username']))) {
			$username_err = "Ingresa el username.";
			...
			if ($stmt = $mysql_db->prepare($sql)) {
				...
				if ($stmt->execute()) {
					...
					echo "Oops! ${$username}, algo ha ido mal. Por favor, inténtelo de nuevo más tarde.";
				}
	    ...
	    if(empty(trim($_POST["password"]))){
	        $password_err = "Porfavor ingresa la contraseña.";     
	    } elseif(strlen(trim($_POST["password"])) < 6){
	        $password_err = "La Contraseña necesita almenos 6 caracteres";
	    } else{
	    ...
	    if (empty($username_err) && empty($password_err) && empty($confirm_err)) {
			...
			if ($stmt = $mysql_db->prepare($sql)) {
				...
				if ($stmt->execute()) {
					...
					echo "Algo ha ido mal. Por favor, inténtelo de nuevo más tarde.";
				}
...
<body>
	<main>
		<section class="container wrapper">
			<h2 class="display-4 pt-3">Registro</h2>
        	...
        	<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
        		...
        		<p>Ya tienes cuenta? <a href="index.php">Logueate aqui</a>.</p>
        	</form>
		</section>
	</main>
</body>
